...
module.exports = {
   ...
   */
  initialise: function initialise(express, app, hbs) {
    var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {
      initExpress: false,
      initHandlebars: false,
      onReload: []
    };

    var Handlebars = require('./services/Handlebars.js');

    var HotReload = require('./services/HotReload.js');

    var ContentParser = require('./services/ContentParser.js');

    var DefaultResolver = function DefaultResolver(linkInfo) {
      ...
      var url = Models.transform(linkInfo.ref, 'link');
    ...
    if (options.initExpress) {
      ...
      app.use('/assets/core', express["static"](__dirname + "/assets", {
        maxAge: "3600000"
      }));
    } //
    ...
    if (options.initHandlebars) {
      ...
      var onReload = options.onReload && options.onReload.length > 0 ? options.onReload.length ? options.onReload : [options.onReload] : []; // initialise handlebars and attach hot reload.

      Handlebars.initialise(app, hbs, process.cwd()).then(function () {
        HotReload.start({
          onReload: onReload
        });
      });
